				...
				function(err, result)
				{
					...
					console.log(activity_list);
						connection.execute(
							...
							"where AP.ACTIVITY_ID = UA.ADD_TIME AND AP.USER_ID = A.USER_ID AND AP.ACTIVITY_ID in "+
							"(SELECT UA.ADD_TIME "+
							"FROM APP_USER A, BUSINESS B, USER_ACTIVITY UA "+
							"WHERE B.BUSINESS_ID = UA.BUSINESS_ID AND UA.USER_ID = A.USER_ID AND UA.USER_ID != :user_id) ",[userProfile.id],
							function(err, result2)
							{
								...
								res.render('friendsactivities', { activitylist: activity_list, namelist: name_list, userid: userProfile.id, userGivenName: userProfile.name.givenName});
							});
					});
				});	
